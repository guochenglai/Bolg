...
import net.gcl.blog.business.service.BizTagsService;
import net.gcl.blog.business.vo.TagsConditionVO;
import net.gcl.blog.framework.exception.BaseException;
import net.gcl.blog.persistence.beans.BizArticleTags;
import net.gcl.blog.persistence.beans.BizTags;
import net.gcl.blog.persistence.mapper.BizArticleTagsMapper;
import net.gcl.blog.persistence.mapper.BizTagsMapper;
...
public class BizTagsServiceImpl implements BizTagsService {
    ...
    public Tags insert(Tags entity) {
        ...
        if (this.getByName(entity.getName()) != null) {
            throw new BaseException("标签添加失败，标签已存在！[" + entity.getName() + "]");
        }
    ...
    public boolean removeByPrimaryKey(Long primaryKey) {
        ...
        if (!CollectionUtils.isEmpty(articleTags)) {
            throw new BaseException("当前标签下存在文章信息，禁止删除！");
        }
    ...
    public boolean updateSelective(Tags entity) {
        ...
        if (old != null && !old.getId().equals(entity.getId())) {
            throw new BaseException("标签修改失败，标签已存在！[" + entity.getName() + "]");
        }
        entity.setUpdateTime(new Date());
